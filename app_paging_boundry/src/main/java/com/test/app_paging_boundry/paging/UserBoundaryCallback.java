...
import com.test.app_paging_boundry.api.RetrofitClient;
...
public class UserBoundaryCallback extends PagedList.BoundaryCallback<User> {
                ...
                .enqueue(new Callback<List<User>>() {
                    ...
                    public void onResponse(Call<List<User>> call, Response<List<User>> response) {
                        if(response.body() != null){
                            List<User> users = response.body(); // 網路查來的資料
                            // 插入數據
                            addUsers(users);
                        }
                    }
    ...
    // 插入數據
    private void addUsers(final List<User> users){
        AsyncTask.execute(() -> {
